  ...
  useEffect(() => {
    ...
    async function loadMargins() {
      ...
      try {
        setLoading(true);
        setStatus("");
        const feed = filter === "friends" ? await getFriendsMarginsFeed() : await getMarginsFeed();
        if (active) setEntries(feed);
      } catch (error) {
        console.error("Could not load The Margins:", error);
        if (active) setStatus("We couldn't load The Margins.");
      } finally {
      ...
      <div className="stack-lg">
        ...
        {filter === "groups" && (
          <section className="panel margins-coming-soon">
            <Users size={22} />
            <div>
              <strong>Groups feed</strong>
              <p className="muted">Groups will be added in the next social phase.</p>
            </div>
          </section>
        )}
