...
\n  async function refreshSocial() {\n    const [loadedFriends, loadedIncoming, loadedOutgoing] = await Promise.all([\n      getFriends(),\n      getIncomingFriendRequests(),\n      getOutgoingFriendRequests()\n    ]);\n    setFriends(loadedFriends);\n    setIncomingRequests(loadedIncoming);\n    setOutgoingRequests(loadedOutgoing);\n  }\n\n  async function handleReaderSearch(event) {\n    event?.preventDefault();\n    try {\n      setSocialBusy(true);\n      setReaderResults(await searchReaders(readerSearch));\n    } catch (error) {\n      console.error("Could not search readers:", error);\n      setStatus("We couldn't search readers.");\n    } finally {\n      setSocialBusy(false);\n    }\n  }\n\n  async function handleSocialAction(otherUserId, action) {\n    try {\n      setSocialBusy(true);\n      if (action === "send") await sendFriendRequest(otherUserId);\n      if (action === "cancel") await cancelFriendRequest(otherUserId);\n      if (action === "accept") await respondToFriendRequest(otherUserId, true);\n      if (action === "decline") await respondToFriendRequest(otherUserId, false);\n      if (action === "remove") await removeFriend(otherUserId);\n      await refreshSocial();\n      if (readerSearch.trim()) setReaderResults(await searchReaders(readerSearch));\n    } catch (error) {\n      console.error("Friend action failed:", error);\n      setStatus("We couldn't update that friendship.");\n    } finally {\n      setSocialBusy(false);\n    }\n  }\n\n  function openEditProfile() {
    setEditingProfile(
      true
    );
...
\n        {activeTab === "friends" && (\n          <section className="panel profile-panel">\n            <div className="section-heading-row"><div><p className="eyebrow">Reader Community</p><h2>Friends</h2></div></div>\n\n            <form onSubmit={handleReaderSearch} style={{ display: "flex", gap: "0.6rem", marginBottom: "1.25rem" }}>\n              <input type="search" value={readerSearch} onChange={(event) => setReaderSearch(event.target.value)} placeholder="Find readers by display name" style={{ flex: "1 1 auto" }} />\n              <button type="submit" className="button primary" disabled={socialBusy || readerSearch.trim().length < 2}><Search size={16} /> Search</button>\n            </form>\n\n            {readerResults.length > 0 && (\n              <div className="public-profile-entry-list">\n                {readerResults.map((result) => (\n                  <article key={result.id} className="public-profile-entry">\n                    <Link to={`/read/public/${result.id}`} className="public-entry-book-title">{result.displayName || "Reader"}</Link>\n                    {result.about && <p className="muted">{result.about}</p>}\n                    <button type="button" className="button secondary" disabled={socialBusy} onClick={() => handleSocialAction(result.id, "send")}><UserPlus size={16} /> Add Friend</button>\n                  </article>\n                ))}\n              </div>\n            )}\n\n            {incomingRequests.length > 0 && (\n              <>\n                <h3>Friend Requests</h3>\n                <div className="public-profile-entry-list">\n                  {incomingRequests.map((request) => (\n                    <article key={request.id} className="public-profile-entry">\n                      <Link to={`/read/public/${request.otherUserId}`} className="public-entry-book-title">{request.profile?.displayName || "Reader"}</Link>\n                      <div className="button-row">\n                        <button type="button" className="button primary" disabled={socialBusy} onClick={() => handleSocialAction(request.otherUserId, "accept")}><UserCheck size={16} /> Accept</button>\n                        <button type="button" className="button secondary" disabled={socialBusy} onClick={() => handleSocialAction(request.otherUserId, "decline")}>Decline</button>\n                      </div>\n                    </article>\n                  ))}\n                </div>\n              </>\n            )}\n\n            {outgoingRequests.length > 0 && (\n              <>\n                <h3>Sent Requests</h3>\n                <div className="public-profile-entry-list">\n                  {outgoingRequests.map((request) => (\n                    <article key={request.id} className="public-profile-entry">\n                      <Link to={`/read/public/${request.otherUserId}`} className="public-entry-book-title">{request.profile?.displayName || "Reader"}</Link>\n                      <button type="button" className="button secondary" disabled={socialBusy} onClick={() => handleSocialAction(request.otherUserId, "cancel")}>Cancel Request</button>\n                    </article>\n                  ))}\n                </div>\n              </>\n            )}\n\n            <h3>Your Friends</h3>\n            {friends.length === 0 ? <p className="muted">You haven't added any friends yet.</p> : (\n              <div className="public-profile-entry-list">\n                {friends.map((friend) => (\n                  <article key={friend.id} className="public-profile-entry">\n                    <Link to={`/read/public/${friend.otherUserId}`} className="public-entry-book-title">{friend.profile?.displayName || "Reader"}</Link>\n                    {friend.profile?.about && <p className="muted">{friend.profile.about}</p>}\n                    <button type="button" className="button secondary" disabled={socialBusy} onClick={() => handleSocialAction(friend.otherUserId, "remove")}><UserMinus size={16} /> Remove Friend</button>\n                  </article>\n                ))}\n              </div>\n            )}\n          </section>\n        )}\n\n        {activeTab === "margins" && (
          <section className="panel profile-panel">
            <div className="section-heading-row">
              <div>
                <p className="eyebrow">
