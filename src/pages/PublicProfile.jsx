  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      try {
        ...
        if (loadedProfile && auth.currentUser && auth.currentUser.uid !== String(userId)) {
          try {
            const relationship = await getFriendshipStatus(userId);
            if (active) setFriendship(relationship);
          } catch (friendError) {
            console.error("Could not load friendship status:", friendError);
          }
        }
      ...
      <div className="stack-lg">
        ...
        <section className="profile-header-card">
          ...
          <div className="profile-header-content">
            ...
            {auth.currentUser && auth.currentUser.uid !== String(userId) && (
              <div className="button-row" style={{ marginTop: "1rem" }}>
                {friendship.status === "none" && (
                  <button type="button" className="button primary" disabled={friendshipBusy} onClick={() => handleFriendAction("send")}>
                    <UserPlus size={16} /> Add Friend
                  </button>
                )}
                {friendship.status === "outgoing" && (
                  <button type="button" className="button secondary" disabled={friendshipBusy} onClick={() => handleFriendAction("cancel")}>
                    <Clock3 size={16} /> Request Sent
                  </button>
                )}
                {friendship.status === "incoming" && (
                  <>
                    <button type="button" className="button primary" disabled={friendshipBusy} onClick={() => handleFriendAction("accept")}>
                      <UserCheck size={16} /> Accept
                    </button>
                    <button type="button" className="button secondary" disabled={friendshipBusy} onClick={() => handleFriendAction("decline")}>Decline</button>
                  </>
                )}
                {friendship.status === "friends" && (
                  <button type="button" className="button secondary" disabled={friendshipBusy} onClick={() => handleFriendAction("remove")}>
